...
with open('CurV 0gk 0.14V (10%) low and high pass flat stereo mode 5 Ohms.csv') as csvfile_1:
    ...
    for row in pm:
        ...
        if (0.000016 < float(row[3])) and (float(row[3]) < 0.000027):
...
with open('ResisV 0gk 0.14V (10%) low and high pass flat stereo mode 5 Ohms.csv') as csvfile_2:
    ...
    for row in pm:
        ...
        if (0.0 < float(row[3])) and (float(row[3]) < 0.000035):
...
with open("Results_10% (5 ohms).txt", "w") as text_file:
...
plt.savefig("V vs. t (10%, 5 ohms).pdf")