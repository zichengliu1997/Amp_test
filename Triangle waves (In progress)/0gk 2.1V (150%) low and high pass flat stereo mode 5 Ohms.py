...
with open('CurV 0gk 2.1V (150%) low and high pass flat stereo mode 5 Ohms.csv') as csvfile_1:
...
with open('ResisV 0gk 2.1V (150%) low and high pass flat stereo mode 5 Ohms.csv') as csvfile_2:
    ...
    for row in pm:
        ...
        if (-0.0002 < float(row[3])) and (float(row[3]) < 0.00025):
            x4.append(float(row[3])*1000)
            resv10pc_y2.append(float(row[4])*10)

csvfile_2.close()

with open('PrimV 0gk 2.1V (150%) low and high pass flat stereo mode 5 Ohms.csv') as csvfile_3:
